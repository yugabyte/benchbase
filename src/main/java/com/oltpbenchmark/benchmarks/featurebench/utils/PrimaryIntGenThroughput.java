...
public class PrimaryIntGenThroughput implements BaseUtil {
    ...
    public PrimaryIntGenThroughput(List<Object> values) {
        if (values.size() != 2) {
    ...
    public PrimaryIntGenThroughput(List<Object> values, int workerId, int totalWorkers) {
        if (values.size() != 2) {
            throw new RuntimeException("Incorrect number of parameters for util function "
                + this.getClass());
        }
