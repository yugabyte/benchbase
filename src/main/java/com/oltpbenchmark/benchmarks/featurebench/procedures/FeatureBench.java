...
package com.oltpbenchmark.benchmarks.featurebench.procedures;
...
public class FeatureBench extends Procedure {
    ...
    public final SQLStmt featureBenchStmt = new SQLStmt(";");

    public void run(Connection conn) {
        try (PreparedStatement stmt = this.getPreparedStatement(conn, featureBenchStmt)) {
            // IMPORTANT:
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Exception for FeatureBench query. This may be expected!", ex);
            }
        }
    }
